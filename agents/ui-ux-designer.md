...
- **CTAに絵文字・アイコンは入れない（テキストのみ）**。配色は記事のパステルに馴染むローズ/ラベンダーで一貫（LINE=ラベンダー・予約=ローズ）。
