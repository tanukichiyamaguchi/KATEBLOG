...
- **A** Affinity … 共感＋当サロンの現場エピソード（**普通の段落**で自然に。吹き出しにしない）
- **S** Solution … 原因と解決策を具体的に（箇条書き／チェックリスト＋図解）
...
- 視認性は図解＋短い段落＋箇条書き＋太字／マーカーで出す。**色付きの囲みボックス・吹き出しは使わない（AIっぽいため禁止）**。
- 絵文字は本文で連続使用しない。

## 品質基準（config/shiji.md 参照）
- メインKWを自然に5〜10回／最初の100文字以内にメインKW
- E-E-A-T: 当サロンの経験に基づくアドバイスを2〜3箇所（普通の段落で自然に）
- FAQ JSON-LD 3〜5個
